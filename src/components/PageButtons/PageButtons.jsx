...
const PageButtons = ({formState, setFormState, pageCount, setPageCount}) => {
    ...
    const pageCheckPrev = parseFloat(formState.startIndex)-parseFloat(formState.maxResults)
    const pageNext = (parseFloat(formState.startIndex)+parseFloat(formState.maxResults))
    ...
    return (
        <div className={styles.PageButtons}>
            {prevButton && <button className={styles.PageButtons__ButtonPrev} onClick={previousPage}>Previous Page</button>}
            <p className={styles.PageButtons__Count}>Page {pageCount}</p>
            {nextButton && <button className={styles.PageButtons__ButtonNext} onClick={nextPage}>Next Page</button>}
        </div>
    )
}
